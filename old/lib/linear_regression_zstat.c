...
double linear_regression(double* X, double* Y, int N) {
    ...
    NCyy = sum_ysq - (sum_y * sum_y) / N;
    NCxy = sum_xy  - (sum_x * sum_y) / N;
    b1 = NCxy / NCxx;
    mean_sq_residual = (NCyy - (b1 * NCxy)) / (N - 2);
    error_b1 = sqrt(mean_sq_residual / NCxx);
    /**
    b0 = (sum_y - (b1 * sum_x)) / N;
...
//      int snps = 5;
//  
//      double chisqarr[gene * snps];
//      double xarr[snps * donr];
//      double yarr[gene * donr];
//  
//      for (i = 0; i < snps * donr; i++) {
//          xarr[i] = i + rand() % 20;
//      }
//      for (i = 0; i < gene * donr; i++) {
//          yarr[i] = i + rand() % 20;
//      }
//      for (i = 0; i < gene * snps; i++) {
//          chisqarr[i] = 0;
//      }
//  
//      nsample = donr;
...
//      ngene = gene;
//  
//      //fit(double* genotype, double* expression, int* nsnps, int* ngene, int* ndonor, double* chisqarr)
//      fit(xarr, yarr, nsnps, ngene, nsample, chisqarr);
//  
//      for (i = 0; i < snps; i++) {
//          printf("SNP %d:", i + 1);
//          for (j = 0; j < gene; j++) {
//              printf("%g ", chisqarr[i * gene + j]);
//          }
//          printf("\n");
//      }
//  
